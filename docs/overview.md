...
```
ProjectFilePath: File path for         Project: Mapping of file paths to
an Elm module                          module contents/ASTs
             |                                       |
             '--------------------+------------------'
                                  |
                                  v
Elm.AST: Abstract syntax tree for the module, generated by `stil4m/elm-syntax`.
                                  |
                                  v
ProgramInterface: Extract Elm AST fragments relevant to using the Elm program.
                                  |
                                  v
ProgramInterop: Convert Elm ASTs to a second AST which represents only types
that can be safely transfered through flags and ports.
                                  |
                                  v
ProgramDeclaration: TypeScript code fragments to slot into a declarations file
template.
                                  |
                                  v
Final output is a string, which is passed to Node and written to disk.
```
